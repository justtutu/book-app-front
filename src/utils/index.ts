...
/**
 * 获取页面顶部安全距离，兼容微信小程序胶囊按钮和安全区
 * @param extraGap 额外间距，默认8px
 */
export function getPageMarginTop(extraGap = 8) {
  // #ifdef MP-WEIXIN
  const menuButtonRect = uni.getMenuButtonBoundingClientRect()
  if (menuButtonRect && menuButtonRect.top && menuButtonRect.height)
    return `${menuButtonRect.top + menuButtonRect.height + extraGap}px`
  const systemInfo = uni.getWindowInfo()
  if (systemInfo.safeArea && systemInfo.safeArea.top)
    return `${systemInfo.safeArea.top}px`
  // #endif
  // 其他端
  return '0px'
}
